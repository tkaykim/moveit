...
-- 적용: Supabase MCP apply_migration (name: t1_membership_orders_schema)
-- 규칙: DROP/DELETE/TRUNCATE 없음. 기존 컬럼 타입 변경 없음.
-- staff 술어는 기존 패턴(is_academy_staff(academy_id) OR is_super_admin())을 그대로 따른다.
...
-- ============================================================
-- 2. memberships (ticket_coverage 보다 먼저 필요하지 않지만 FK 순서 정리)
-- ============================================================
...
create index if not exists ticket_coverage_class_group_id_idx
  on public.ticket_coverage (class_group_id);

-- ============================================================
-- 4. membership_discounts
-- ============================================================
create table if not exists public.membership_discounts (
  ...
  -- class_group_id / ticket_id 중 정확히 하나만 non-null
  constraint membership_discounts_target_xor_chk check (
    (class_group_id is not null)::int + (ticket_id is not null)::int = 1
  )
);

create index if not exists membership_discounts_membership_id_idx
  on public.membership_discounts (membership_id);
create index if not exists membership_discounts_class_group_id_idx
  on public.membership_discounts (class_group_id);
create index if not exists membership_discounts_ticket_id_idx
  on public.membership_discounts (ticket_id);
...
-- 학원별 사용자당 ACTIVE 멤버십 1개만
create unique index if not exists student_memberships_active_uniq
  on public.student_memberships (academy_id, user_id)
  where status = 'ACTIVE';
...
create index if not exists order_groups_user_created_idx
  on public.order_groups (user_id, created_at desc);
create index if not exists order_groups_academy_status_created_idx
  on public.order_groups (academy_id, status, created_at);
create index if not exists order_groups_pending_expiry_idx
  on public.order_groups (status, expires_at)
...
create table if not exists public.order_items (
  ...
  constraint order_items_discount_percent_chk check (
    discount_percent is null or discount_percent between 1 and 100
  ),
  constraint order_items_ticket_purchase_chk check (
    item_type <> 'TICKET_PURCHASE' or ticket_id is not null
  ),
  constraint order_items_schedule_booking_chk check (
    item_type <> 'SCHEDULE_BOOKING' or schedule_id is not null
  ),
...
create index if not exists order_items_order_group_id_idx
  on public.order_items (order_group_id);
create index if not exists order_items_schedule_id_idx
  on public.order_items (schedule_id);
...
create index if not exists booking_events_status_created_idx
  on public.booking_events (status, created_at);

-- ============================================================
-- 9. 기존 테이블 확장 (ADD COLUMN only)
-- ============================================================
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'tickets_start_mode_chk'
  ) then
    alter table public.tickets
      add constraint tickets_start_mode_chk check (start_mode in ('IMMEDIATE', 'FIRST_BOOKING'));
  end if;
end $$;
...
begin
  if not exists (select 1 from pg_constraint where conname = 'user_tickets_fixed_class_id_fkey') then
    alter table public.user_tickets
      add constraint user_tickets_fixed_class_id_fkey
      foreign key (fixed_class_id) references public.classes(id);
  end if;
  if not exists (select 1 from pg_constraint where conname = 'user_tickets_source_membership_id_fkey') then
    alter table public.user_tickets
      add constraint user_tickets_source_membership_id_fkey
      foreign key (source_membership_id) references public.memberships(id);
  end if;
end $$;
...
begin
  if not exists (select 1 from pg_constraint where conname = 'bookings_order_group_id_fkey') then
    alter table public.bookings
      add constraint bookings_order_group_id_fkey
      foreign key (order_group_id) references public.order_groups(id);
  end if;
  -- T0 에서 FK 없이 추가된 classes.audience_membership_id 에 이제 FK 부여
  if not exists (select 1 from pg_constraint where conname = 'classes_audience_membership_id_fkey') then
    alter table public.classes
      add constraint classes_audience_membership_id_fkey
      foreign key (audience_membership_id) references public.memberships(id);
  end if;
end $$;

create index if not exists bookings_order_group_id_idx
  on public.bookings (order_group_id);

-- ============================================================
-- 10. RLS — 8개 신규 테이블 전부 enable, default deny + 최소 권한
-- ============================================================
...
-- memberships: staff/service 전용 (hidden 멤버십 누출 금지 — anon SELECT 없음)
...
-- membership_discounts: staff/service 전용 (academy 는 memberships 경유)
...
create policy membership_discounts_all_staff on public.membership_discounts
  for all to authenticated
  using (
    is_super_admin() or is_academy_staff(
      (select m.academy_id from public.memberships m where m.id = membership_discounts.membership_id)
    )
  )
  with check (
    is_super_admin() or is_academy_staff(
...
-- booking_events: staff/service 전용
...
-- ticket_coverage: 로그인 사용자 SELECT 허용(커버리지 표시용), 쓰기는 staff
...
create policy ticket_coverage_write_staff on public.ticket_coverage
  ...
  using (
    is_super_admin() or is_academy_staff(
      (select t.academy_id from public.tickets t where t.id = ticket_coverage.ticket_id)
    )
  )
  with check (
    is_super_admin() or is_academy_staff(
...
-- student_memberships: 본인 또는 학원 staff SELECT, 쓰기는 staff
...
-- order_groups: 본인 또는 학원 staff SELECT, 쓰기는 staff/service
...
-- order_items: order_groups 소유자 또는 학원 staff SELECT, 쓰기는 staff/service
  ...
  using (
    exists (
      ...
      where og.id = order_items.order_group_id
        and (og.user_id = auth.uid() or is_academy_staff(og.academy_id) or is_super_admin())
    )
  );
...
create policy order_items_write_staff on public.order_items
  ...
  using (
    exists (
      select 1 from public.order_groups og
      where og.id = order_items.order_group_id
        and (is_academy_staff(og.academy_id) or is_super_admin())
    )
  )
  with check (
    exists (

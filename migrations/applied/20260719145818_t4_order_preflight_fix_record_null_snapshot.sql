-- 수정: 복합 record 에 대한 `IS NOT NULL` 은 "모든 필드가 not null" 을 의미한다.
-- tickets/classes 는 nullable 컬럼이 많아 항상 false 였고, 그 탓에
-- ticket_name_snapshot / ticket_type_snapshot / start_mode_snapshot / class_id 가
-- 전부 null 로 기록됐다(= 스냅샷 불변성이 실제로는 비어 있었다).
-- 명시적 found 플래그로 교체한다.

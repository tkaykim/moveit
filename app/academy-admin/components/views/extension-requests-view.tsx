...
import { Check, X, Loader2, Pause, Calendar } from 'lucide-react';
...
export function ExtensionRequestsView({ academyId }: ExtensionRequestsViewProps) {
  ...
  const loadRequests = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/ticket-extension-requests?academyId=${academyId}`);
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400">
        회원이 신청한 연장/일시정지를 승인하거나 거절합니다. 거절 시 사유를 입력해야 합니다.
      </p>

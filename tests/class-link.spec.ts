...
test('L5 로그아웃 방문자는 이 자리에서 간편가입 시트로 유도되고, 로그인 폴백은 같은 회차 복귀(next)를 보존한다', async ({
  page,
  context,
}) => {
  await context.clearCookies();
  ...
  // Q-간편가입: 로그아웃 CTA 는 이제 페이지를 떠나지 않고 이 자리에서 시트를 연다.
  const signin = page.getByTestId('class-signin');
  await expect(signin).toBeVisible();
  await signin.click();
  await expect(page.getByTestId('quick-join-sheet')).toBeVisible();
  await expect(page).toHaveURL(new RegExp(`/c/${F.schedBookable.id}$`));

  // 로그인 폴백 → 같은 회차로 되돌아가는 next 를 보존한다.
  await page.getByTestId('quick-join-to-login').click();
  await page.waitForURL(/\/my\?next=/, { timeout: 20000 });
  await expect(page.getByTestId('my-login')).toBeVisible();
  const url = new URL(page.url());
  expect(decodeURIComponent(url.searchParams.get('next') || '')).toContain(classUrl(F.schedBookable.id));

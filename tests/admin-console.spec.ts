...
test.beforeAll(async () => {
  ...
  // 회차 — 명단용(가까운 미래) / 먼 미래(보강 대상·검토 큐용)
  //
  // ⚠ 명단(roster)은 **KST 달력일**로 필터한다(getRoster → kstDayRangeUtc).
  //   회차 시각을 now+6h 로 두면 (예약창이 열려 있어 홀드 생성은 항상 성공하지만)
  //   KST 저녁에는 그 시각이 **다음 KST 일자**로 넘어가 기본 선택일(오늘)과 어긋난다
  //   → 하루 중 시간대에 따라 명단이 비어 실패했다.
  //   해결: 시각은 now+6h(항상 예약창 열림) 그대로 두되, 그 시각의 **실제 KST 날짜**를
  //   기록하고 테스트에서 날짜 선택기를 그 날짜로 몰아준다. 어느 시간대에도 일치한다.
  const todayScheduleStart = isoInHours(6);
  F.rosterDate = kstDateString(todayScheduleStart);
  F.scheduleToday = await ins('schedules', {
    class_id: F.classReady.id,
    start_time: todayScheduleStart,
...
test('명단: 새 주문 경로로 들어온 예약이 수강권·미입금 상태와 함께 보인다', async ({
  ...
  // 픽스처 회차가 놓인 KST 날짜로 선택기를 명시적으로 몰아준다.
  // (기본값은 렌더 시점의 KST 오늘 — beforeAll 과 테스트 실행이 KST 자정을 사이에 두면
  //  하루가 어긋날 수 있어, 픽스처가 기록한 날짜로 고정한다.)
  const datePicker = page.getByTestId('roster-date');
  await expect(datePicker).toBeVisible({ timeout: 20_000 });
  await datePicker.fill(F.rosterDate);
